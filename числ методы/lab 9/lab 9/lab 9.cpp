...
double third_d(double x) {
    return pow(2, x) * x * pow(log(2), 3) + 3 * pow(2, x) * pow(log(2), 2);
}

double fourth_d(double x) {
    return pow(2, x) * x * pow(log(2), 4) + 4 * pow(2, x) * pow(log(2), 3);
}
...
void error_estimate(double error, int power, double point, vector<double> x) {
    ...
    case 3: {
        double w=1;
        double Mn = fourth_d(x[x.size() - 1]);
        for (int i = 0; i < x.size(); i++) {
            if (fourth_d(x[i]> Mn)) Mn= fourth_d(x[i]);
        }
        analytical_error = Mn / factorial(power + 1);
            ...
            else cout <<"aaaaaaa" << w<< " "<< analytical_error/w << " " << analytical_error;
        }
          break;
    case 2: {
        double w = 1;
        double Mn = third_d(x[x.size() - 1]);
        for (int i = 0; i < x.size(); i++) {
            if (third_d(x[i] > Mn)) Mn = third_d(x[i]);
        }
        analytical_error = Mn / factorial(power + 1);
...
int main() {
    ...
    vector<double> y = generate_function_values(x, function);
    cout << "Y: ";
    print_vector(y);
    ...
    try {
        ...
        vector<double> gap_y = generate_function_values(gap, function);
        ...
        double error_l = fabs(function(point) - value_l);
        cout << "Значение многочлена Лагранжа степени " << nl << " в точке " << point << ": "
            << value_l << endl;
        cout << "Значение функции в точке " << point << ": " << function(point) << endl;
        ...
        gap_y = generate_function_values(gap, function);
        cout << "Используемые значения:" << endl;
        cout << "X: ";
        print_vector(gap);
        cout << "Y: ";
        ...
        double error_n = fabs(function(point) - value_n);
