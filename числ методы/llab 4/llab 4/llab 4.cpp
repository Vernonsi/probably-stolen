    ...
    for (int i = 0; i < str; i++)
    {
        for (int k = 0; k < st; k++)
        {
            ...
            if (k == st - 1) {
                ...
                cout << "Enter element (" << i + 1 << ", " << st << "): ";
                cin >> array[i][st - 1];
            }
            else {

                cout << "Enter element (" << i + 1 << ", " << k + 1 << "): ";
                cin >> array[i][k];
            }
...
bool CheckZero(int str_2, int st, long double** array, int str) {//!=0
...
bool CheckZero2(int str_2, int st, long double** array, int str) {//=0
...
void FirstStroka(int str_2, int st, long double** array, int str) {
    if (array[str][str] == 0 && str != st - 2) {
        ...
        for (int i = str; i < str_2; i++) {
            if (array[i][str] != 0) {
                count_str = i;
                Turn(count_str, st, array, str); //swap
                break;
            }
        }
    }
    if (array[str][str] != 1 && array[str][str] != 0) {
        ...
        for (int i = 0; i < st; i++) {
            array[str][i] = array[str][i] * m; // делим строку на первый член
        }
...
void Raznost(int str_2, int st, long double** array, int str) {
    ...
    if (array[str][st2] == 0) {
        ...
        {
            if (array[str][i] != 0) {
                st2 = i; break; // st2 eto index x
            }
        }
    }
    if (array[str][st2] != 0) {
        for (int i = str + 1; i < str_2; i++) {
            double m = array[i][st2] / array[str][st2]; // delim stolbik s x na x
            for (int k = 0; k < st; k++)
            {
...
void Roots(long double** array, int str, int st) { // vichisl korni
...
int re(long double** array, int str, int st) { // kol-vo pustih strok
    int count = 0;
    int k = 100;
    int d = str - 1;
    for (int i = 0; i < str; i++)
...
int main()
{
    ...
    cout << "\n\n\n\n";
    for (int i = 0; i < str; i++) {
        if (CheckZero(str, st, array, i)) goto beck; // если нулевая строка при не нулевой g то нет решений 
        FirstStroka(str, st, array, i);
        Raznost(str, st, array, i);
        str = str - re(array, str, st); // ubiraem pustie stroki
    }
    OutArray(str, st, array);
    cout << "\n\n\n\n";
